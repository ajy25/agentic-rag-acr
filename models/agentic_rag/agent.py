...
from agentic_rag.config import SQLITE_DB_PATH, settings
...
class RagResponse(BaseModel):
    answer: str = Field(
        ..., description="Final response grounded in the knowledge base"
    )
...
def build_agent() -> Agent:
    ...
    instructions = (
        ...
        " to answer questions. Do not include citations or source references in your replies."
    )
    return Agent(
        name="acr-agent",
...
@cli.command()
def chat(
    context_k: int = typer.Option(0, help="Preview this many chunks each turn")
) -> None:
    agent = build_agent()
